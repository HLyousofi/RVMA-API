...
class StoreWorkOrderProductRequest extends FormRequest
{
    ...
    {
        return [
            'productsWorkOrder' => 'required|array',
            'productsWorkOrder.*.product_id' => 'required|integer|exists:products,id',
            'productsWorkOrder.*.quantity' => 'required|integer|min:1',
            'productsWorkOrder.*.unit_price' => 'required|numeric|min:0',
        ];
    }
    ...
    {
        return [
            'productsWorkOrder.required' => 'At least one product is required.',
            ...
            'productsWorkOrder.*.quantity.required' => 'The quantity is required.',
            'productsWorkOrder.*.quantity.min' => 'The quantity must be at least 1.',
            'productsWorkOrder.*.unit_price.required' => 'The price is required.',
